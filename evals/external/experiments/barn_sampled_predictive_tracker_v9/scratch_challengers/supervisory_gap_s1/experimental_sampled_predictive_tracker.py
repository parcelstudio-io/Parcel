"""Deployment-disabled supervisory local tracker for the V9 scratch lane.
...
advance until measured odometry reaches a bounded displacement.  Every frame
is revalidated.  The unchanged raw 720-ray V8 shield remains the final action
authority; this tracker neither replaces nor weakens it.
"""
...
class SampledPredictiveTracker:
    ...
    def reset(self) -> None:
        ...
        self._settled = False
        self._unknown_commitment_ticks = 0
    ...
    def select(
        ...
        *,
        ...
        if self._committed_heading_world_rad is not None:
            return self._continue_escape(pose=pose, scan=prepared, goal_world=goal_world)
        ...
        if nominal_vx < 0.0 or abs(nominal_vy) > 1e-12:
            ...
            self._last_vx_mps = 0.0
            self._last_yaw_rate_rps = 0.0
            return self._hold("nominal_outside_forward_only_contract")
    ...
    def _target_heading_error(
        ...
        *,
        ...
        if waypoint is not None:
            ...
            value = math.atan2(goal_world[1] - pose.y, goal_world[0] - pose.x) - pose.heading_rad
    ...
    def _nominal_is_admitted(
        ...
        rollout_safe = self._rollout_is_safe(vx_mps, yaw_rate_rps, scan.obstacle_xy)
        return (
            observed and decision.output_vx_mps >= self.minimum_translation_mps and rollout_safe,
            decision,
        )
    ...
    def _start_escape(
        ...
        *,
        ...
        if choice is None:
            ...
            self._last_vx_mps = 0.0
            self._last_yaw_rate_rps = 0.0
            return self._hold("no_fully_observed_gap", extra=f"trigger={trigger}")
    ...
    def _continue_escape(
        ...
        *,
        ...
        if state == "unknown":
            self._unknown_commitment_ticks += 1
            self._last_vx_mps = 0.0
            self._last_yaw_rate_rps = 0.0
            if self._unknown_commitment_ticks <= self.unknown_commitment_grace_ticks:
                return self._hold(
                    "committed_gap_temporarily_unobserved",
                    extra=f"unknown_ticks={self._unknown_commitment_ticks}",
                )
            self._clear_commitment(keep_previous=True)
            target = _wrap_angle(
                math.atan2(goal_world[1] - pose.y, goal_world[0] - pose.x) - pose.heading_rad
            )
            return self._start_escape(
                pose=pose,
                scan=scan,
                target_heading_error=target,
        ...
        if state == "blocked":
            ...
            self._last_vx_mps = 0.0
            self._last_yaw_rate_rps = 0.0
            return self._hold("committed_gap_obstructed")

        if not self._settled:
            if abs(heading_error) > self.align_exit_rad:
                return self._rotate_toward_commitment(
                    pose=pose,
                    scan=scan,
                    choice=_GapChoice(heading_error, 0, clearance),
                    trigger="commitment_reused",
                )
            self._settling = True
        ...
        desired_yaw = max(
            ...
            min(self.escape_max_yaw_rate, heading_error * self.yaw_gain),
        )
        ...
        if not self._sweep_is_observed(0.0, yaw * self.reaction_horizon_s, scan):
            ...
            self._last_vx_mps = 0.0
            self._last_yaw_rate_rps = 0.0
            return self._hold("escape_forward_sweep_unobserved", extra="unknown_ticks=1")
        ...
        if decision.output_vx_mps < self.minimum_translation_mps or not self._rollout_is_safe(
            ...
            self._last_vx_mps = 0.0
            self._last_yaw_rate_rps = 0.0
            return self._hold("escape_translation_became_obstructed")
        self._last_vx_mps = decision.output_vx_mps
    ...
    def _rotate_toward_commitment(
        ...
        *,
        ...
        if abs(heading_error) <= self.align_exit_rad:
            ...
            desired = math.copysign(self.min_align_yaw_rate, heading_error)
        ...
        *,
        ...
        for run in runs:
            ...
            target_index = min(
                interior,
                key=lambda index: (
                    abs(_wrap_angle(float(self._gap_bearings[index]) - target_heading_error_rad)),
                    abs(float(self._gap_bearings[index])),
                    index,
                ),
            )
            center_index = run[len(run) // 2]
            # Prefer the goal-clamped interior sample, while a weak center term
            # and width bonus prevent repeated commitment to a gap edge.
    ...
    def _hold(self, reason: str, *, extra: str = "") -> MidLevelCommand:
        ...
        return MidLevelCommand(
            ...
            vyaw=0.0,
            stop=False,
            note=f"v9s1_supervisor_hold reason={reason}{suffix}",
        )
