"""Deployment-disabled supervisory local tracker for the V9 S2 scratch lane.
...
class SampledPredictiveTracker:
    ...
    def select(
        ...
        *,
        ...
        route_recovered = (
            route_available and self._detour_originated_from_route_loss is True
        )
        progress_released_latch = (
            self._detour_latch_goal_distance_m is not None
            and goal_distance_m
            <= self._detour_latch_goal_distance_m - self.detour_latch_progress_m + 1e-12
        )
        if route_recovered or progress_released_latch:
    ...
    def _hold(self, reason: str, *, extra: str = "") -> MidLevelCommand:
        ...
        return MidLevelCommand(
            ...
            vyaw=0.0,
            stop=False,
            note=f"v9s2_supervisor_hold reason={reason}{suffix}",
        )
