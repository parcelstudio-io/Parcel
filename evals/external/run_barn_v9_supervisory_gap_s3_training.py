"""Run the frozen supervisory-gap S3 candidate on its 10 training worlds.

This wrapper has no development, holdout, 100-world, or candidate-path code
path.  It authenticates the S3 scratch freeze, exact content-addressed bundle,
...
from .barn_v9_supervisory_gap_s3 import (
...
DEFAULT_DESCRIPTION = (
    "Training-only 10-world screen of frozen supervisory-gap S3 258e9a33 against exact "
    "rejected V8 control 189ac31f; failed-escape feedback, strict side commitments, "
    "bounded active perception, live route guidance, and evidence-based latch release "
    "are the only candidate factor; no development or holdout access."
)
...
CANDIDATE_IDENTITY = shared.V9TrainingCandidateIdentity(
    ...
    experiment_id="barn-v9-training-supervisory-gap-s3-258e9a33",
    description="Exact deployment-disabled supervisory-gap S3 scratch candidate",
...
def _preflight_training_inputs() -> shared.V9TrainingPreflight:
    ...
    if verification != expected:
        raise shared.V9TrainingRunnerError(
            "verified S3 training manifest/corpus identity is not exact"
        )

    frozen = verify_supervisory_gap_s3()
    ...
    if delta != frozen.plan.delta:
        raise shared.V9TrainingRunnerError(
            "materialized supervisory-gap S3 bundle differs from its frozen plan"
        )
    ...
    reference_spec = parcel_isolated_bundle_reference_spec(
        ...
        description="Exact rejected V8 189ac31f experimental control for S3 training",
    )
...
def run_training_screen(
    *,
    ...
    """Run the only predeclared S3 screen: training IDs 5000--5009."""

    if world_count not in ALLOWED_WORLD_COUNTS:
        raise shared.V9TrainingRunnerError(
            "supervisory-gap S3 world_count must be exactly 10"
        )
