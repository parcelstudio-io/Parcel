"""Run the frozen supervisory-gap S1 candidate on its 10 training worlds.

This wrapper has no development or holdout code path.  It authenticates the
S1 scratch freeze, exact content-addressed bundle, complete 100-world training
corpus, and V8 control before delegating to the shared immutable V9 training
runner.  The resulting row is rerunnable scratch evidence only.
"""
...
from .barn_v9_supervisory_gap_s1 import (
...
DEFAULT_DESCRIPTION = (
    "Training-only 10-world screen of frozen supervisory-gap S1 841597cd against exact "
    "rejected V8 control 189ac31f; nominal passthrough, stable observed-gap commitment, and "
    "distance-bounded escape are the only candidate factor; no development or holdout access."
)
...
CANDIDATE_IDENTITY = shared.V9TrainingCandidateIdentity(
    ...
    experiment_id="barn-v9-training-supervisory-gap-s1-841597cd",
    description="Exact deployment-disabled supervisory-gap S1 scratch candidate",
...
def _preflight_training_inputs() -> shared.V9TrainingPreflight:
    ...
    if verification != expected:
        raise shared.V9TrainingRunnerError(
            "verified S1 training manifest/corpus identity is not exact"
        )

    frozen = verify_supervisory_gap_s1()
    ...
    if delta != frozen.plan.delta:
        raise shared.V9TrainingRunnerError(
            "materialized supervisory-gap S1 bundle differs from its frozen plan"
        )
    ...
    reference_spec = parcel_isolated_bundle_reference_spec(
        ...
        description="Exact rejected V8 189ac31f experimental control for S1 training",
    )
...
def run_training_screen(
    *,
    ...
    """Run the only predeclared S1 screen: training IDs 5000--5009."""

    if world_count not in ALLOWED_WORLD_COUNTS:
        raise shared.V9TrainingRunnerError("supervisory-gap S1 world_count must be exactly 10")
