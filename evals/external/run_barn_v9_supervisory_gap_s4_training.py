"""Run the frozen supervisory-gap S4 candidate on its 10 training worlds.

This wrapper has no development, holdout, 100-world, or candidate-path code
path.  It authenticates the S4 scratch freeze, exact content-addressed bundle,
...
from .barn_v9_supervisory_gap_s4 import (
...
DEFAULT_DESCRIPTION = (
    "Training-only 10-world screen of frozen supervisory-gap S4 3c739663 against exact "
    "rejected V8 control 189ac31f; failed-escape feedback, bounded active perception, "
    "ordered route guidance, yaw-slew-safe holds, and evidence-based latch release are "
    "the only candidate factor; no development or holdout access."
)
...
CANDIDATE_IDENTITY = shared.V9TrainingCandidateIdentity(
    ...
    experiment_id="barn-v9-training-supervisory-gap-s4-3c739663",
    description="Exact deployment-disabled supervisory-gap S4 scratch candidate",
...
def _preflight_training_inputs() -> shared.V9TrainingPreflight:
    ...
    if verification != expected:
        raise shared.V9TrainingRunnerError(
            "verified S4 training manifest/corpus identity is not exact"
        )

    frozen = verify_supervisory_gap_s4()
    ...
    if delta != frozen.plan.delta:
        raise shared.V9TrainingRunnerError(
            "materialized supervisory-gap S4 bundle differs from its frozen plan"
        )
    ...
    reference_spec = parcel_isolated_bundle_reference_spec(
        ...
        description="Exact rejected V8 189ac31f experimental control for S4 training",
    )
...
def run_training_screen(
    *,
    ...
    """Run the only predeclared S4 screen: training IDs 5000--5009."""

    if world_count not in ALLOWED_WORLD_COUNTS:
        raise shared.V9TrainingRunnerError(
            "supervisory-gap S4 world_count must be exactly 10"
        )
