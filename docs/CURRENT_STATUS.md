...
| Emotion poses and self-returning gestures | Implemented | Catalogued; personality affect maps can queue the gesture trajectories when idle | Catalog/package/prompt and coordinator tests; simulator only | `attentive_stand`, `relaxed_crouch`, `comfort_bow`, `happy_wiggle`, `attentive_nod`, and `curious_look` are hardware-unverified. The physical runtime rejects direct pose/trajectory writes while Sport owns locomotion. |
