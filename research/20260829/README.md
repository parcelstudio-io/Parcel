...
**Final-report status:** pending completion and verification of DSOAK-1, final
guarded regression, and fresh independent review. Hosted Model B candidate-Q
absolute gates are complete and refuted; its quota-truncated direct arm and
calibrated human comparison remain explicitly unmeasured. LHO-1 is complete.
Interim soak counters are not results.
...
still represented by [`SOL_METHODICAL_ASSESSMENT_DRAFT.md`](SOL_METHODICAL_ASSESSMENT_DRAFT.md)
until the pending work above closes.
...
| Acoustic loop | 5/9 gates in three runs | Endpoint, stop, acknowledgment, and prosody remain red before mounted audio/AEC |
...
| DMC-4 composed journal | Two identical 1,824-transition runs; 256/256 corruptions; 32-producer concurrency and bounded-overflow gates passed; disarmed runtime-frame composition passed 26 focused tests | Owner journal → process-local Model-B frames is wired; live speech epoch, persistent cursor, provider, audio, and child-resume lineage remain absent |
...
| Stop-only safety | SOS-1 passed two 256-case source/fake-gateway runs plus independent verification | Adopt the source-level principal; real STOP inputs, target timing, and hardware qualification remain absent |
...
| Final guarded shards | Mount 659/4 skip; DMC/runtime 159; conversation 419; duplex/acoustic 264/1 skip; social navigation 279/7 skip; portability 485 | Desktop/injected evidence only; repository-wide gate and independent review pending |
...
| DMC-4 | [`duplex-transaction-4/`](duplex-transaction-4/) | Complete; `DMC4_COMPOSED_PASS` for the frozen source transaction; the additive [`runtime-journal-composition/`](runtime-journal-composition/) wires disarmed process-local frames, while live-session/provider/audio integration remains red |
...
| SOS-1 | [`stop-only-safety-1/`](stop-only-safety-1/) | Complete; independently verified source/fake-gateway stop-only principal passes, but real STOP inputs and target qualification are absent |
| DSOAK-1 | [`duplex-soak-1/`](duplex-soak-1/) | Preregistered 12-hour wall-clock run in progress; no interim result admitted |
...
Today, learned Model A is not integrated. DMC-4 adds a bounded owner-authored
...
not bind speech generation to a live session epoch, persist a restart-safe
cursor, carry separate-child resume lineage, or deliver provider/audio context. The [one-page code
map](../../docs/PRODUCTION_RUNTIME_CODE_MAP.md) shows the exact current source
boundaries.

## Immediate priorities

1. Finish and independently verify DSOAK-1; run the final guarded regression
   and fresh independent review. Do not assign a hosted Q-minus-D effect or
   calibrated human failure rate from the quota-truncated MB-1 arm.
...
4. Fix the four acoustic gates, then test physical mic/speaker/AEC replay and
   Starlink loss/jitter while local safety remains independent.
