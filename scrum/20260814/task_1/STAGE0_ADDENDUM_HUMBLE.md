# Stage-0 command addendum — ROS 2 HUMBLE sheet (card S-2)

> # ⚠ DRAFT UNTIL H-1 — NOT YET OPERATIVE
>
> Nobody has executed a command on the Orin. Its ROS distro is an **assertion, not an observation**. Two sheets are generated: this one for ROS 2 **humble**, and one for ROS 2 **jazzy**.
>
> **Exactly one becomes operative** when the operator reports the observed distro from the H-1 identity dump (`cat /etc/nv_tegra_release; lsb_release -a; ls /opt/ros`). If H-1 reports `/opt/ros/humble`, this document is the sheet and `STAGE0_ADDENDUM_JAZZY.md` is **VOID**. If it reports `/opt/ros/jazzy`, **this document is VOID** and `STAGE0_ADDENDUM_JAZZY.md` is the sheet.
>
...
| | |
|---|---|
| ROS 2 distro (DRAFT — H-1 confirms) | `humble` |
...
**EXPECTED.** `grep -c` prints **0**. The file names the interface `ip -brief addr` printed, and `CYCLONEDDS_URI` is an absolute `file://` URI. The `<Interfaces><NetworkInterface name=…>` schema is Cyclone 0.10+, which is what ROS 2 humble ships.
...
4. **The humble recorder CLI facts are read from ros2/rosbag2's source, not executed on Humble by anyone.** T10.1 exists precisely because that gap cannot be closed from a desk.
