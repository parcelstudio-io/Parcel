...
|---|---|---|---|
| A1 | **M1-0 GATEWAY — DONE, ACCEPTED** (M1_0_STATUS.md; 135 tests incl. the 50 Hz soak verified in my own guard run; committed 6bbb49e; label was stale — fixed at RRV-1) | HLD Gate 1; ARCH-1 X12 (co-located); bridge fixtures. **Added from Codex's live-code note: hung-vendor-call isolation** — `stop_move`/`state`/ledger I/O must be bounded or contained so a hung call cannot freeze the watchdog; fault set gains hung-I/O, lease theft, second writer, slow client, audit-full (HLD Gate 1 list) | seeded-fault suite green ×3; exact-zero on every loss class; watchdog provably non-blocking |
| A2 | **NAV-GLUE — DONE, ACCEPTED; TOPOLOGY DECIDED: SIMPLIFY** (A2_VERDICT_FABLE.md; committed 6511afd; the M1 nav acceptance row — shipped configuration ≥0.80 on the frozen corpus — dispatched as NAV-ACCEPT at RRV-1; label was stale — fixed at RRV-1) | Fix 3: ONE clearance authority (0.42 m planner vs 0.752/0.80 m brakes; 8/8 stalls "inside a brake ring, route still planned"). **Measured refinement: this is a DESIGN change, not a parameter wire** — `planner_inflation_m` has no call site and `_planner_coupling_ring_m` caps tighter-only by design (pending DOOR-1 H-2); `map_safety_margin_m`=0.45 recovered only 1/8 sampled stalls. A2 owns the DOOR-1 H-2 decision + a brake→replan signal. Fix 1: region/object kind-tolerant learned-map query (12/12 `bed` episodes `not_found`). Fix 2: off-oracle arrival verification (metric band + detector confirmation; `target_surface_unobserved` on 15/60). Then **re-run the frozen NAV-CORE corpus unchanged**: arm A ≥ 0.80 ⇒ retain; only arm B ⇒ simplify (fixes 1–2 stay post-M1) | corpus re-run decides retain/simplify; N4 typed-failure = 1.00 |
| A3 | **DISCONTINUITY-LATCH — DONE, ACCEPTED** (A3_VERDICT_FABLE.md; kidnap-onset row fired, jump 2.64 m journalled; margin flag ships OFF pending the M1 nav corpus run; runtime installer → A4) | NAV-CORE refuter 4b: shipped arms kept translating on HEALTHY after a kidnap (824–840/840 HEALTHY ticks, 0.84 m moved); the modelled A4/A10 latch caught 3/3 and the operator pose-reset re-armed cleanly; the whole-map second-best margin **does not exist in the product** (`ScanMatchLocalizer._relocalize` keeps no runner-up — fix 4) and correctly refused in the aliased world (margins 0.002–0.03 vs 0.25). Build: the latch + trigger journal + runner-up margin + the operator reset transaction. Also fix 5: no arrival claim from uncalibrated covariance (R3's false arrival at p=0.9922, 0.534 m out) — arrival confidence needs a calibration floor or detector confirmation | latch regression (kidnap ⇒ 0.00 m); margin published; R3 case refuses; **kidnap-ONSET detection in a NORMAL layout fires the jump/mismatch (JUMP_BOUND) path — not only ambient ambiguity — with `localization_jump_m` journalled** (4b lens); the operator re-arm is a ONE-SHOT journalled transaction, never a standing authorization |
| A4 | **SPINE — DONE, ACCEPTED** (A4_VERDICT_FABLE.md; K3 9→0, K4 exists, K6 7; A2/A3 handoffs discharged; Gate-4 native-read cutover, wire codec, RangeTemplateSource, real LIO, operator re-arm route = recorded follow-ups) | `NavigationSnapshotV2` assembler replacing `SimObservation` in the 9 audited modules (EMBODIMENT-KERNEL list: brain/observations, control/base, control/state, navigation/{follow, reactive_safety, search_owner, semantic_map, spatial}, runtime.py); stamped evidence header (HLD principle 5); simulator/replay/physical adapters; Orin service packaging skeleton | K3 = 0 modules; K4 = exists; adapters pass the existing suites |
| A5 | **C8-FIX — DONE, ACCEPTED; the C8 BUILD_BLOCKER is cleared** (A5_VERDICT_FABLE.md; structural cancel_now guard, HOLD-first transaction, arbiter-verified quiescence, command-stream regression; connected replanning unblocked — the CONNECTED-PLANNER probe still awaits the owner's compound decision) | Addendum A8: transactional suspend-only, atomic/rollback, `_amendment_pending` gated on arbiter quiescence, HOLD on partial failure; regression observes the command stream | multi-task forced-failure regression green; r24/nominal-stop unchanged |
| A6 | **STOP-LOCAL — DONE, ACCEPTED** (A6_VERDICT_FABLE.md; always-local Silero→whisper→whole-word path onto the panel's own latch, pinned by call spy; `name_prefixed` DEFAULT behind the `stop_hotword.mode` knob, hybrid/bare implemented; tail bars met on the replay tier p95 608/785 ms, bare's 864/24 h reproduced; ≤1/24 h unfalsified ≈87/24 h bound; through-air/mounted = box-day; no `barge_in` on the safety thread — follow-up filed) | Addendum A9 tail bars: p95 ≤ 800 ms AND n ≥ 60 all ≤ 1.0 s; false triggers ≤ 1/24 h; today's only cloud-independent stops are panel/remote/watchdogs (lane.py:47) | bars met on the desk array (VOICE-GATE measures) |
| A7 | **EAR + GOVERNOR — DONE, ACCEPTED** (A7_VERDICT_FABLE.md; measured rate card into the SpendLedger — +335.75% mispricing closed, itemized v2 rows; `EarGate` = the only route from frame to wire: press → governor → identity 0.352/≥2 s channel-matched → pre-roll ≥500 ms → upload, refused turns erased; `triage_in_exchange` wired 66→8 hear_only; `HostedCallGovernor` $160+$40 with typed refusals, critical never reads the ledger, safety structurally out of the money path; ambient OFF; live-probe items + 0.55 arming recalibration = box-day) | H1: rate card into the runtime `SpendLedger` (old ledger overcharged +336 %); pre-roll ≥ 500 ms (0 % truncation); server VAD stays ON (silence not billed — proven for server-VAD sessions only); **identity/engagement gate BEFORE upload** (Codex freeze finding 3: post-upload identity is too late for cost and privacy; H1 C5 960 opens/h on TV); `triage_in_exchange` (66/174 owner turns misread context-free); **product hard-cap call governor** (EVENT-BUDGET: nominal p95 $30.72/mo, ungated TV $571/mo, 1 Hz tick $777/mo — the governor is what is missing, not the price) | C-row regressions; governor seeded-red (cap reached ⇒ refuse non-critical) |
| A8 | **FOLLOW-COMPOSE — DONE, ACCEPTED** (A8_VERDICT_FABLE.md; `install_owner_tracker` has product callers behind `owner_follow.tracker` (off ships; uncalibrated gallery refused, P1-C 0.9295); pixel/range one-epoch sync with derived bound `min(TTLs)`; ambiguity VETOES the controller ⇒ safe-HOLD + line, loss ⇒ HOLD + reacquire window from owner-search's own timeout; follow speed under the A2 single authority by identity; A3's `motion_latched` now truthfully published; A6 STOP preempts follow onto the panel's latch; **UWB DEFERRED out of the M1 BOM from measurement** (0/100 vision swaps, beacon 0.66–0.78 at the crossing vs σ 0.25 m; AoA bar ≥0.95 @ ≥0.75 m + measured NLOS = the re-open trigger); ENABLE stays box-day-gated — `_owner_identity_commissioned()` is necessary, never sufficient) | tracker product installation (`install_owner_tracker` has zero callers), synchronized pixel/range via the A4 snapshot, UWB decision from measured two-person ambiguity (not preference), follow-speed avoidance; ambiguity/loss ⇒ HOLD + canned line | capability suite + the box-day identity gate decides ENABLE |
| A9 | **MEMORY-FIXES + NON-TRAVEL LIFE — DONE, ACCEPTED; LANE A COMPLETE** (A9_VERDICT_FABLE.md; H5's four defects fixed at the named sites with pre-A9 arms still reproducing each — tombstone-aware upsert + `respect_revocations` on the write path, session-id filter real, `complete_json` constrained mode off the cancellable path, label-strength estimator for unconfigured maps only; H4 productized as `motion/body_lane.py` beneath the dispatch chain — HOLD-as-command, same-tick e-stop, no actuator route; H3 under `attention/initiative.py`'s STRUCTURAL zero-translation lease — 6/h envelope, same-tick yield, safe-hold terminals with `returned=True` raising the refuted numbers; initiative SHIPS OFF with no product trigger yet; scheduler product caller + soak/nuisance bar + adapters = Gate 8's other half, box-day) | H5's four verified defects (session-id filter, chat-completions proposer, tombstone-aware upsert, ranking margin); body composer (H4: 50 Hz, HOLD-as-command, e-stop 0.88 tick) + drives (H3: 5.3/h, 90 % admitted, 0-tick yield; **travel radius 0** — 1,222 contacts refuted travel; and per the ratified wave-2 terminal-aware refutation, the initiated-leg terminal is a **safe-hold invariant + receding-horizon against live people-flow**, NOT a scripted stop-and-return, which measured worse: contacts 319→323, contact time 89→245 s) under the zero-translation lease | governance suite + soak + the human nuisance bar |
