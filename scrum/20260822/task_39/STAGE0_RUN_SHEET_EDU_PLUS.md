<!-- HW-8: was "# Stage 0 run-sheet — dry-run (motion disabled)" -->
# Stage 0 run-sheet — **Go2 EDU Plus with Mid-360** — dry-run (motion disabled)
...
**Superseding authority for the sequencing:** [PHYSICAL_SESSION_PLAN.md](../../20260813/task_1/PHYSICAL_SESSION_PLAN.md)
**Channel enumeration:** [CHANNEL_MATRIX.md](../../20260813/task_1/CHANNEL_MATRIX.md) — **25 rows / 28 channels** (rewritten by PS-H; §7 C0.3's 19-row table is completed in §7A)
**Safety brief (read aloud before power):** [SAFETY_BRIEF.md](../../20260813/task_1/session/SAFETY_BRIEF.md)
**Mount geometry (fill while the rig is assembled):** [MOUNT_GEOMETRY_SHEET.md](../../20260813/task_1/session/MOUNT_GEOMETRY_SHEET.md)
**Photographs:** [PHOTO_LIST.md](../../20260813/task_1/session/PHOTO_LIST.md) · **Takes (what actually gets recorded):** [TAKE_SCRIPT.md](../../20260813/task_1/session/TAKE_SCRIPT.md) — T0–T16, added by PS-K; read **§7A below** before §3

**Card:** PS-F, tranche PS-1 · **Written:** 2026-08-13 · **Author:** Fable (PS-F)

<!-- HW-8: was nothing — this block is the whole reason a second copy exists. -->
> ## ⬜ THIS IS THE EDU PLUS REWRITE — card HW-8, 2026-08-23
>
> Source: [`scrum/20260813/task_1/session/STAGE0_RUN_SHEET.md`](../../20260813/task_1/session/STAGE0_RUN_SHEET.md)
> (sha256 `f874ed46e7456b2f9fcf34622ea2601dc116d83e65310e79347b71021b4b3df7`),
> **copied, never edited**. Every substantive change below carries a
> `<!-- HW-8: was "…" -->` comment so the diff is reviewable line by line.
> Relative links were rebased for this file's new location — one mechanical
> pass, declared here rather than commented forty times.
>
> **What the purchase changed:** the Orin NX is **onboard** in the robot's own
> AI docking station, not a separate dock on a desk; the external LiDAR is a
> **Livox Mid-360**, not an add-on Unitree L2, and it lands on the dock's M8
> air plug — **UNCONFIRMED**, inferred from third-party cable listings, and
> settled by step **Q-wire** of `docs/BOX_DAY.md` before this sheet's T1 runs;
> the recorder runs on **that** Orin; the port census and the mount follow the
> dock's own USB complement. Everything about safety, stop bars, branches,
> evidence discipline and the take script is the 08-13 sheet's and is
> deliberately unchanged.
>
> **Three changes beyond the purchase**, kept because they are improvements,
> each commented in place: (1) §3's T1–T6 "source of truth" column now names
> the module and its `--help` instead of the PS-\*_STATUS.md docs — those
> cards landed and the modules are real (card Work 5); (2) §5 gains the PO-1
> e-stop block and the Q-stop independence measurement — PO-1 was taken after
> 08-13 and its record gates the first armed step; (3) row 19's XVF3800 moves
> from AWAITING to ON HAND, a fact change. Everything else is purchase-forced.
>
> **Read [`docs/BOX_DAY.md`](../../../docs/BOX_DAY.md) first.** Its first two
> hours (B9 identity, B-fw firewall, S20 firmware, Q-dev, Q-lidar, Q-wire,
> Q-usb) happen **before** this sheet starts, and B9's JetPack branch can stop
> the day before this sheet is opened at all.
>
> **Design of record:** [`scrum/20260822/WAVE3_HW_DESIGN_FABLE.md`](../WAVE3_HW_DESIGN_FABLE.md) §5.3, §5.7, §7.
...
4. If something cannot be measured, write `NOT MEASURED — <reason>`. Never
   ...
   value.
5. Anyone may call **"STOP"** at any time. See [SAFETY_BRIEF.md](../../20260813/task_1/session/SAFETY_BRIEF.md) §5.
...
<!-- HW-8: was "gnss=____________ xvf=____________" on the second device_ids
     line. The ZED-F9P is on PO-1's don't-buy list (indoor companion); the
     Livox Mid-360 ships in the EDU Plus bundle and is the LiDAR this session
     records. `dock=` is now the robot's ONBOARD AI docking station. -->
```text
...
device_ids:       go2=____________ dock=____________ d455=____________
                  mid360=____________ xvf=____________
...
<!-- HW-8: was "l2_addon_serial: / l2_addon_firmware:". There is no add-on
     Unitree L2 in this purchase. The external LiDAR is a Livox Mid-360 that
     lands on the dock's M8 air plug (UNCONFIRMED wiring — inferred from
     third-party cable listings; step Q-wire of docs/BOX_DAY.md settles the
     NIC, the subnet and the M8 voltage before this sheet's T1 runs). -->
† Additions required by this session and not present in the ratified template:

```text
mid360_serial:        ____________________
mid360_firmware:      ____________________
mid360_nic:           ____________________   # which Jetson NIC sees Livox UDP :56300 (Q-wire)
mid360_ip:            ____________________   # Livox convention 192.168.1.1xx, host 192.168.1.5
m8_plug_volts:        ____________________   # METER, POWERED OFF FIRST. Mid-360 wants 9-27 V
...
orin_serial:          ____________________   # the ONBOARD Orin NX 16 GB, inside the dock
...
|---|---|---|---|---|
| T1 | Preflight + attestation | `python3 -m scripts.parcel_capture.preflight --help` then `…attest --help` — **verified to exist 2026-08-23, HW-8** | `______________________________________` | ____ |
| T2 | Budget check for planned duration | `python3 -m scripts.parcel_capture.budget --help`, [BANDWIDTH_BUDGET.md](../../20260813/task_1/BANDWIDTH_BUDGET.md) | `______________________________________` | ____ |
| T3 | Clock map (start burst → run → end burst) | `python3 -m scripts.parcel_capture.clockmap --help`; the ritual card is `python3 -m scripts.parcel_capture.syncevents --ritual-card` | `______________________________________` | ____ |
| T4 | Record | `python3 -m scripts.parcel_capture.record --help`. **The `ros2 bag record` argv is not typed by hand**: render it with `python3 -m scripts.parcel_capture.stage0_addendum --print-argv humble` <!-- HW-8: was "--distro <humble|jazzy> --emit-distro". `--emit-distro` WRITES a sheet into scrum/20260814/task_1/ inside the checkout, and git on the Orin is read-only by this sheet's own rule; `--print-argv` prints the same argv from the same renderer and writes nothing. --> | `______________________________________` | ____ |
| T5 | Sidecar emit + verify | `python3 -m scripts.parcel_capture.sidecar --help`; readback is `…record --verify <bag>.mcap` | `______________________________________` | ____ |
| T6 | Rehearsal replay (dry, on the Orin, before the dog) | `python3 -m scripts.parcel_capture.rehearse --help`; the no-dog phase harness is `…orin_rehearsal --help` (its runbook: `scrum/20260814/task_1/ORIN_RUNBOOK.md`) | `______________________________________` | ____ |

<!-- HW-8: was the PS*_STATUS.md status docs as the "source of truth" column.
     Those cards landed; the modules are real and answer --help, so the source
     of truth is now the module itself. Every spelling above was run on
     2026-08-23 and exited 0 (HW8_STATUS.md, Work 5). Two commands the design
     document spells DO NOT EXIST and are deliberately absent from this sheet:
     `parcel-capture …` is not a console script (there is no `parcel-capture`
     in pyproject.toml; use `python3 -m scripts.parcel_capture.<module>`), and
     `record --plan stage0 --dry-run` has no --plan, no stage0 and no
     --dry-run. The readiness check is `record --check`. -->
...
|---|---|---|---|
| P0 | Owner authorized purchase; BOM received + inventoried | MET / NOT MET | <!-- HW-8: was "Hardware is on hand (Go2 EDU, L2, D455, Orin NX)." -->Hardware is on hand (Go2 **EDU Plus with Mid-360**, D455, XVF3800 array; the **Orin NX 16 GB is onboard**, inside the robot's own AI docking station — there is no separate dock to inventory). **The BOM receipt log is still empty** — fill `P5_PROCUREMENT_BOM.md` receipt table with serials at this session. Serials also go in §1. |
| P1 | Go2 firmware ≥1.1.13 recorded; auto-update disabled | MET / NOT MET | <!-- HW-8: was "PS-D attestation reads it **off the unit**." That is not true of this tree: `attest` ships no live identity reader (`preflight.probe_robot_identity` falls to `_unavailable_device_reader("go2")`), so the version comes back ABSENT and the session verdict is REFUSE_CONNECT. The real route is the app read, recorded by the harness. --> **The version is read in the Unitree app and recorded by `orin_rehearsal --until p3_network --firmware-attested V<x.y.z>`** (step S20 of `docs/BOX_DAY.md`); no software in this repository can read it off the unit. Below pin ⇒ **hard refusal**, §6 branch DEGRADE-MMP, and **do not attach the Orin to the robot LAN** (ADR 0002: pre-pin firmware is RCE-capable on that LAN). Auto-update disabled: operator confirms in the vendor app, records screenshot ID ______ |
| P2 | Sacrificial Orin dock flashed to pinned JetPack 6.2.x; image hash recorded | **WAIVED-FOR-SENSOR-ONLY** | <!-- HW-8: the dock is now the robot's own onboard carrier, so "sacrificial" is not merely unmet — it is unattainable, and the JetPack question is answered by step B9 of docs/BOX_DAY.md before this sheet opens. --> **No flash is performed today.** The JetPack the unit ships with is **UNCONFIRMED** (2024–25 EDU docks are reported as JetPack 5.1.1 / Ubuntu 20.04 / CPython 3.8 [documented]); if B9 read JetPack 5, this sheet does not run at all — see `docs/BOX_DAY.md` "The JetPack-5 branch". Record the JetPack version the Orin already carries (PS-D attestation) as an observation, not as a validated golden image. Consequence: **ADR 0001 stays unvalidated and P5-G-INSTALL cannot close from this session.** |
| P3 | Second (production) dock restored from golden image | **CANNOT BE MET** | <!-- HW-8: was "BOM line 2 specifies qty 2 Orin docks; one is on hand." The EDU Plus integrates the Orin into the robot, so a second dock now means a second robot. --> The EDU Plus's Orin is **integral to the robot**: a second dock is a second $17k machine, not a $2k accessory. The two-dock rule (`adr/0001-golden-image.md:19-21`) is not exercisable. Consequence: the only dock present is therefore **not** sacrificial — **do not flash it, do not `apt upgrade` it, do not mutate it** during this session. Installing anything on it during the session is an OWNS violation of this sheet. |
| P4 | Dual e-stop hardware present and labeled | MET / NOT MET | §5 is the verification. Serials + photo → [PHOTO_LIST.md](../../20260813/task_1/session/PHOTO_LIST.md) P14. **NOT MET ⇒ the dog is not powered.** |
...
**Read [SAFETY_BRIEF.md](../../20260813/task_1/session/SAFETY_BRIEF.md) aloud before this section. All hands
clear of the leg envelope.**

Name the two **independent** stops. They must not share a battery, a radio
link, or a failure mode.

<!-- HW-8: added. On 08-13 the two stops were an open question on this sheet.
     PO-1 (scrum/20260822/task_27/README.md) now names the decision that must
     be RECORDED before any --arm: (a) the Unitree handheld remote plus a
     leash, with an explicit written waiver of docs/MOTION.md:441-442, or
     (b) a battery-path relay (hardware mod, warranty). Nothing on this sheet
     arms anything, so the record is not a precondition for THIS session — it
     is a precondition for the first armed step, and the measurement that
     decides whether (a) is honest is taken here. -->
> **The independence of stop #1 is a MEASUREMENT, not an assumption.** The
> remote's `L2` (long press) + `B` is Damping / emergency stop [documented];
> whether that damping acts through the MCU **independently of the head board
> and DDS** is **UNCONFIRMED**. Step **Q-stop** of
> [`docs/BOX_DAY.md`](../../../docs/BOX_DAY.md) is the test: on the stand,
> sport mode on, with the head board's NIC unplugged, press it and see whether
> the dog damps. Record the result here before writing an independence
> argument. If it does not damp, the remote is **not** an independent stop and
> PO-1 must be re-decided before anything is armed.

```text
q_stop_result (head-board NIC unplugged, remote L2+B):  DAMPED / DID NOT DAMP / NOT TESTED
po1_e_stop_decision_recorded:  YES / NO   (required before any --arm, not before this session)
```
...
| Critical channels `PRESENT` | PS-D vs [CHANNEL_MATRIX.md](../../20260813/task_1/CHANNEL_MATRIX.md) | at minimum ch.1 built-in LiDAR, ch.5 sportmodestate, ch.6 lowstate, ch.12–15 D455 | ______ | [ ] |
...
- the **mount geometry** ([MOUNT_GEOMETRY_SHEET.md](../../20260813/task_1/session/MOUNT_GEOMETRY_SHEET.md)) — gone the moment the bracket is unbolted;
- the **photographic record** ([PHOTO_LIST.md](../../20260813/task_1/session/PHOTO_LIST.md)) — including the built-in LiDAR's model label, which settles the L1-vs-L2 contradiction in the repo without any software at all.
...
| <!-- HW-8: added. The onboard Orin is multi-homed — robot LAN, Mid-360 subnet, Wi-Fi 6/4G WAN — so it is the ONLY thing with a foot in both worlds, and it is where the boundary is enforced (design §5.7). --> **The Orin is the boundary** (new on EDU Plus): default-drop forwarding between the robot-LAN NIC and **every** WAN interface (Wi-Fi 6, 4G, second RJ45); DDS multicast confined to the robot NIC; robot NIC carries **no default route**; panel bound to `127.0.0.1` and reached over the tailnet | **Operator action** — step B-fw of `docs/BOX_DAY.md`, done **before** any WAN interface came up | `nft list ruleset` saved? Y/N ____ · `ip route` shows no default via robot NIC? Y/N ____ |
| <!-- HW-8: added — the Mid-360's documented address convention collides with a common home LAN. --> **Mid-360 subnet does not collide with the home LAN** — Livox convention is host `192.168.1.5`, lidar `192.168.1.1xx` [documented]; `configs/robot.yaml` already flags this collision | **Operator action** (Q-wire) | home LAN /24: ____________ · Mid-360 /24: ____________ · collide? Y/N ____ |
| **Clock** | **PS-C `clockmap.py`** — and read the note below | clockmap started ______ UTC, ended ______ UTC, digest ____________ |
| **TF** | **NOT APPLICABLE this session** — there is no TF implementation in Parcel (`pose.py:74-78` is a 2-member `Frame` enum with no transform function). Today's substitute is the **measured** extrinsic in [MOUNT_GEOMETRY_SHEET.md](../../20260813/task_1/session/MOUNT_GEOMETRY_SHEET.md). | geometry sheet filled? Y/N ____ |
...
[CHANNEL_MATRIX.md](../../20260813/task_1/CHANNEL_MATRIX.md) — **28 channels; the 19 rows below are incomplete — §7A carries the rest** — and a channel that
...
| 10 | **Mid-360 point cloud** — Livox UDP `:56300` on the Mid-360 NIC, **not** DDS | ~10 Hz, 200 k pts/s [documented] | ______ | ______ | ______ |
| 11 | **Mid-360 IMU** (ICM40609) — Livox UDP `:56400` | ~200 Hz | ______ | ______ | ______ |
...
| 17 | GNSS ZED-F9P | <!-- HW-8: was "CONFIRM on hand" — PO-1 puts the ZED-F9P + NTRIP on the don't-buy list for an indoor companion. --> **NOT PURCHASED** (PO-1 don't-buy) | ABSENT (expected) | — | — |
| 18 | UWB owner fob | CONFIRM on hand | ______ | ______ | ______ |
| 19 | XVF3800 mic array | <!-- HW-8: was "AWAITING (in the post) / ABSENT (expected)" — the array arrived and is the one piece of hardware on hand today. --> **ON HAND** — probe with `python3 tools/xvf3800_probe.py --json` (step Q-usb) | ______ | ______ | ______ |

<!-- HW-8: rows 10 and 11 were "add-on L2 cloud ~10-20 Hz" and "add-on L2 IMU
     ~200 Hz". There is no add-on Unitree L2 in this purchase and no
     unilidar_sdk2 path. The Mid-360 is read over raw Livox UDP (cmd 56100,
     point 56300, IMU 56400 [documented]) or livox_ros_driver2; whether that
     driver is preinstalled on the dock is UNCONFIRMED (Q-wire). The decoder
     that turns those frames into `lidar_ranges` is card HW-3. -->
D455 profile actually selected (a **budget decision**, not a default — see
[BANDWIDTH_BUDGET.md](../../20260813/task_1/BANDWIDTH_BUDGET.md)):
...
and in what order — are in **[TAKE_SCRIPT.md](../../20260813/task_1/session/TAKE_SCRIPT.md)**, T0–T16, and the
...
**If the day is cut short**, [TAKE_SCRIPT.md](../../20260813/task_1/session/TAKE_SCRIPT.md) §5 is the explicit
...
| <!-- HW-8: added. With a Mid-360 fitted, which unit feeds `voxel_map` and whether the two LiDARs interfere is an open design question (Q-lidar of the unknowns register). Record both, decide nothing here. --> 23b | **head LiDAR vs Mid-360 coexistence** — with both powered, do the head cloud rates in rows 1/21 change? Is `voxel_map_compressed` (row 4) still fed, and by which unit? | observation, not a rate | ______ | ______ | ______ |
...
descriptions in [SAFETY_BRIEF.md](../../20260813/task_1/session/SAFETY_BRIEF.md) §3.
...
>    ([SAFETY_BRIEF.md](../../20260813/task_1/session/SAFETY_BRIEF.md) H4), and the first unplanned posture
...
>    everyone expecting it** — as [TAKE_SCRIPT.md](../../20260813/task_1/session/TAKE_SCRIPT.md) take **T2c**,
...
| S1 | <!-- HW-8: was "Payload security — Orin, D455, L2, brackets, battery". The Orin is inside the dock and not a separate payload item; the Mid-360 and the array are. --> **Payload security** — D455, **Mid-360**, XVF3800 array, brackets, cables | Each item cannot be moved by a firm two-finger pull in **any** axis. Every fastener seated; nothing held by tape or friction alone. | ______ |
...
| **S11** | **The deliberate standing-stop drop test has been done** — §9 E1′, over the mat, **with the recorder running** ([TAKE_SCRIPT.md](../../20260813/task_1/session/TAKE_SCRIPT.md) take T2c) | No take that relies on the payload staying on a **standing** dog may run before this. Damage found afterwards is recorded, and the payload is re-checked from S1. | ______ |
...
> **This whole section is [TAKE_SCRIPT.md](../../20260813/task_1/session/TAKE_SCRIPT.md) take T2c**, and it is
...
> ([TAKE_SCRIPT.md](../../20260813/task_1/session/TAKE_SCRIPT.md) §2): the press is timestamped on
...
- [ ] **Take T14 SYNC-CLOSE recorded BEFORE power-down** — [TAKE_SCRIPT.md](../../20260813/task_1/session/TAKE_SCRIPT.md) T14, on the still-bolted, still-powered rig. It is 5 minutes and it is what turns take T3's offset into an offset **plus a drift slope**. If it was skipped, write that in `does_not_prove` in those words.
- [ ] **The take log is filled** — [TAKE_SCRIPT.md](../../20260813/task_1/session/TAKE_SCRIPT.md) §2, one row per take, including the takes that did **not** happen and why
- [ ] **§4A pre-torque FOV verdict recorded** in [MOUNT_GEOMETRY_SHEET.md](../../20260813/task_1/session/MOUNT_GEOMETRY_SHEET.md) — `CONFIRMED` / `MARGINAL` / `NONE` / `NOT DONE`
- [ ] **M2 re-measure done** — the teardown column of [MOUNT_GEOMETRY_SHEET.md](../../20260813/task_1/session/MOUNT_GEOMETRY_SHEET.md) is filled **while the rig is still assembled**. A delta beyond tolerance invalidates the session's extrinsic; you can only know that now.
- [ ] Every **MANDATORY** photograph in [PHOTO_LIST.md](../../20260813/task_1/session/PHOTO_LIST.md) taken and offloaded
...
- **A dry-run record is not a dataset.** Everything §7 closes is evidence that
  ...
  [TAKE_SCRIPT.md](../../20260813/task_1/session/TAKE_SCRIPT.md), and on nothing in §7.
...
<!-- HW-8: added — four things this rewrite specifically cannot claim. -->
- **Nothing on this sheet is evidence about the Mid-360 until Q-wire has run.**
  How the Mid-360 is wired, which NIC carries it, what the M8 plug supplies,
  and whether `livox_ros_driver2` is preinstalled are all **UNCONFIRMED**
  today; they are read in step Q-wire of `docs/BOX_DAY.md`, not here.
- **This sheet cannot prove the head LiDAR's model.** Row 20
  (`utlidar/lidar_state`) and the photographed label settle L1-vs-L2; a
  document does not.
- **A recorder that runs on the onboard Orin is not the same measurement as
  the 08-13 plan's.** Storage, thermals and USB bandwidth are the robot's
  now, and every budget number in
  [BANDWIDTH_BUDGET.md](../../20260813/task_1/BANDWIDTH_BUDGET.md) must be
  re-measured on this machine before it is believed.
- **This sheet still authorises no motion**, and the EDU Plus changes nothing
  about that. The first armed step has its own four preconditions in
  `docs/BOX_DAY.md`, including PO-1's recorded e-stop decision and card
  HW-6's envelope re-run with measured numbers.
