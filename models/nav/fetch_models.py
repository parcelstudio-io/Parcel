"""Download and verify pinned navigation-model artifacts."""
...
def fetch_model(name: str, spec: dict[str, Any], *, root: Path = ROOT) -> Path:
    ...
    request = urllib.request.Request(
        str(spec["url"]),
        headers={"User-Agent": "Parcel-model-fetch/1"},
    )
    # A partial download keeps the ``.incomplete`` suffix and is never exposed
    # at the configured checkpoint path.
    with urllib.request.urlopen(request, timeout=60) as response, incomplete.open("wb") as out:
