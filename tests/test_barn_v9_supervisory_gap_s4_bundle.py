...
def test_raw_freeze_mutation_is_rejected(tmp_path: Path) -> None:
    ...
    with pytest.raises(challenger.SupervisoryGapS4Error, match="raw identity changed"):
...
def test_source_mutation_is_rejected_after_freeze(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValueError, match="tracker source bytes differ"):
        challenger.verify_supervisory_gap_s4(experiment_root=changed_root)

    assert challenger.verify_supervisory_gap_s4().freeze_sha256 == (
