...
from evals.external import run_barn_v9_supervisory_gap_s1_training as runner
from evals.external import run_sampled_predictive_tracker_v9_training as shared
from evals.external.barn_v9_supervisory_gap_s1 import (
...
def test_real_preflight_authenticates_s1_bundle_corpus_and_isolated_pair() -> None:
    ...
    assert preflight.isolated_pair["candidate"]["package_sha256"] == CANDIDATE_PACKAGE_SHA256
    assert preflight.isolated_pair["candidate"]["manifest_sha256"] == (CANDIDATE_MANIFEST_SHA256)
...
def test_s1_has_no_100_world_development_holdout_or_path_override(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(
        shared,
