...
def packaged_manifest_path() -> Path:
    """Release-parity record generated by ``tools/sync_runtime_assets.py``."""

    return _PACKAGED_ASSETS / "MANIFEST.json"


def load_packaged_manifest() -> dict:
    """Read the packaged manifest; readable from a checkout and from a wheel."""

    return json.loads(packaged_manifest_path().read_text(encoding="utf-8"))
