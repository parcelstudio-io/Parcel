...
- The plan-level goal describes the final physical outcome. For a sequential
  request such as “go to the sidewalk and then wait by the lamppost,” use one
  separately grounded `NavigateTo` step per target; do not concatenate both
  actions into one directive. The executive rechecks each step's own success
