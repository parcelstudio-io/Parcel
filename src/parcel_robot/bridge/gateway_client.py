"""``MotionGatewayClientV1`` — the product side of the motion seam.
...
going through the Unix socket to the gateway process.  The separate gateway
distribution may depend on and compatibility-re-export this module; the product
must never depend on the gateway package.  ``tests/test_motion_seam.py`` pins
that import surface and the public method set.
