...
def agent_cost_at(
    ...
    *,
    ...
    track_values = np.array(
        [(track.x, track.y, track.vx, track.vy, track.radius_m) for track in tracks]
    )
    centers_x = (track_values[:, 0, None] + track_values[:, 2, None] * times).ravel()
    centers_y = (track_values[:, 1, None] + track_values[:, 3, None] * times).ravel()
    ...
    widths = (sigma_m + np.repeat(track_values[:, 4], len(times))) ** 2
    rollout_weights = np.tile(weights, len(tracks))
    dx = query[:, 0, None] - centers_x
    dy = query[:, 1, None] - centers_y
    lobes = np.exp(-(dx * dx + dy * dy) / (2.0 * widths))
    costs = lobes @ rollout_weights
