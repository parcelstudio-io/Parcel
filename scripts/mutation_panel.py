...
PANEL_REGENERATION_PROVENANCE = (
    ...
    "RE-RUN 2026-08-29 (card C0, FIX-SUBSTRATE-1) as the RECORDED RE-RUN half "
    "of the card's (a)-fix-product / (b)-recorded-re-run choice; the owner was "
    "silent and the card's silence default was applied. WHY (b): the superseded "
    "artifact was written at 20260828T084754Z from a working tree that "
    "reproduces on NO commit -- its five clean-run poses are byte-identical to "
    "f3ecb5c but its authority histogram {agreement: 4, tolerated_boundary: 1} "
    "is not f3ecb5c's {agreement: 5}. The DIFF IS ATTRIBUTED TO a379bf4 "
    "(feat: integrate companion contracts and document robotics design, the "
    "owner's hard-safety / terminal-arrival change), reproduced independently "
    "at single-commit resolution: on f3ecb5c the nav-region_goal-D-15 "
    "(go to the crosswalk) episode stalls at the start (final (0.2815, "
    "0.1603), dtg 2.0685, agreement); on a379bf4 and on HEAD it drives 3.0 m, "
    "ends INSIDE the scorer's crosswalk polygon at (2.8113, 0.7495) (dtg "
    "0.0000) and the SYSTEM refuses the claim, so the row records "
    "authority_disagreement. No terminal-contract refusal fires: the navigator "
    "keeps requesting vx=0.102 m/s and apply_reactive_safety returns zero with "
    "meta='stopped' on 105/200 ticks, leaving the robot 1.5151 m from the POI "
    "point crosswalk_a (3.5, -0.6) against the POI path's 1.50 m point-goal "
    "arrive radius -- a 15 mm miss on a hard radius, and the POI point itself "
    "lies OUTSIDE the crosswalk polygon it is supposed to name (NAV-GEN-1 "
    "VERDICT.md 5.2; card C1 owns that grounding defect). (a) was refused "
    "because every way to make the system claim arrival here weakens the "
    "reactive gate, moves a frozen NAV_INSTRUCT digest, or lands the owner's "
    "uncommitted grid_planner.py -- and the owner's uncommitted diff, applied "
    "alone, only nudges the same stall 3 cm to 1.4996 m, crossing the same "
    "radius by 0.4 mm. COST, written rather than hidden: "
    "no_authority_disagreement disabled as a kill channel from this re-run; "
    "re-armed when D-15 agrees again. A check already red on the clean run is "
    "excluded as a kill channel for every mutant (harness_checks / run_panel), "
    "so this re-run costs the panel that channel exactly as the v2 rot cost it "
    "no_false_arrival -- the difference being that this disable is declared "
    "here and asserted by "
    "tests/test_mutation_panel_freshness.py::"
    "test_red_clean_checks_are_declared_disabled_kill_channels. No mutant is "
    "killed ONLY through no_authority_disagreement (arrival_radius_x2 and "
